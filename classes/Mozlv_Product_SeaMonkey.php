...
class Mozlv_Product_SeaMonkey extends Mozlv_Product_Class {
	...
	protected function get_latest_url( $url, $channel, $platform ) {
		if ( $url == $this->download_URL ) {
			switch ( $platform ) {
				case 'win':
					return parent::get_latest_url( 'https://archive.seamonkey-project.org/releases/%1$s/win32/%2$s/seamonkey-%1$s.%2$s.win32.installer.exe', $channel, $platform );
				case 'win64':
					return parent::get_latest_url( 'https://archive.seamonkey-project.org/releases/%1$s/win64/%2$s/seamonkey-%1$s.%2$s.win64.installer.exe', $channel, $platform );
				case 'mac':
					return parent::get_latest_url( 'https://archive.seamonkey-project.org/releases/%1$s/mac/%2$s/seamonkey-%1$s.%2$s.mac.dmg', $channel, $platform );
				case 'lin':
					return parent::get_latest_url( 'https://archive.seamonkey-project.org/releases/%1$s/linux-i686/%2$s/seamonkey-%1$s.%2$s.linux-i686.tar.bz2', $channel, $platform );
				case 'lin64':
					return parent::get_latest_url( 'https://archive.seamonkey-project.org/releases/%1$s/linux-x86_64/%2$s/seamonkey-%1$s.%2$s.linux-x86_64.tar.bz2', $channel, $platform );
			}
		}
